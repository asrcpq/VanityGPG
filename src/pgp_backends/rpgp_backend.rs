...
use pgp::packet::{
	...
	SecretKey as SecretKeyPacket, SecretSubkey as SecretSubkeyPacket,
	UserId as UserIDPacket,
};
use pgp::ser::Serialize;
use pgp::types::{
	CompressionAlgorithm, KeyVersion, PublicParams, SecretKeyTrait,
	SecretParams, Version,
};
...
use super::{
	sha1_to_hex, ArmoredKey, Backend, CipherSuite, PGPError, UniversalError,
	UserID,
};
...
impl Into<PublicKeyPacket> for PublicKeyPacketConverter {
	...
	fn into(self) -> PublicKeyPacket {
		unsafe {
			std::mem::transmute::<PublicKeyPacketConverter, PublicKeyPacket>(
				self,
			)
		}
	}
...
impl Into<PublicSubkeyPacket> for PublicKeyPacketConverter {
	...
	fn into(self) -> PublicSubkeyPacket {
		unsafe {
			std::mem::transmute::<PublicKeyPacketConverter, PublicSubkeyPacket>(
				self,
			)
		}
	}
...
impl Into<SecretKeyPacket> for SecretKeyPacketConverter {
	...
	fn into(self) -> SecretKeyPacket {
		unsafe {
			std::mem::transmute::<SecretKeyPacketConverter, SecretKeyPacket>(
				self,
			)
		}
	}
...
impl Into<SecretSubkeyPacket> for SecretSubkeyPacketConverter {
	...
	fn into(self) -> SecretSubkeyPacket {
		unsafe {
			std::mem::transmute::<SecretSubkeyPacketConverter, SecretSubkeyPacket>(
				self,
			)
		}
	}
...
impl Backend for RPGPBackend {
	...
	fn get_armored_results(
		...
		let public_subkey_packet: PublicSubkeyPacket =
			PublicKeyPacketConverter::new(
				subkey_type.to_alg(),
		...
		let secret_subkey_packet: SecretSubkeyPacket =
			SecretSubkeyPacketConverter::new(
				public_subkey_packet,
				subkey_secret_params,
			)
			.into();
		let secret_subkey =
			SecretSubkey::new(secret_subkey_packet, subkey_flags);

		let uid_packet = UserIDPacket::from_str(
			Default::default(),
			&uid.get_id().unwrap_or("".to_string()),
		);
		...
		let primary_public_key_packet: PublicKeyPacket =
			PublicKeyPacketConverter::new(
		...
		let primary_secret_key_packet: SecretKeyPacket =
			SecretKeyPacketConverter::new(
				primary_public_key_packet,
				self.secret_params,
			)
			.into();
...
mod rpgp_backend_test {
	use super::{
		Backend, CipherSuite, PublicKeyAlgorithm, PublicKeyPacket,
		PublicKeyPacketConverter, RPGPBackend, UserID,
	};
	...
	fn ed25519_fingerprint_calculation() {
		...
		let timestamp = backend.get_timestamp();
		let public_params = backend.get_public_params();
		let public_key_packet: PublicKeyPacket = PublicKeyPacketConverter::new(
			PublicKeyAlgorithm::EdDSA,
			public_params,
			timestamp,
		)
	...
	fn ed25519_shuffle() {
		...
		let public_params = backend.get_public_params();
		let public_key_packet_before: PublicKeyPacket =
			PublicKeyPacketConverter::new(
				PublicKeyAlgorithm::EdDSA,
				public_params.clone(),
				timestamp_before,
			...
			.into();
		let fingerprint_rpgp_before =
			encode_upper(public_key_packet_before.fingerprint());
		let public_key_packet_after: PublicKeyPacket =
			...
			.into();
		let fingerprint_rpgp_after =
			encode_upper(public_key_packet_after.fingerprint());
