...
use sequoia_openpgp::types::{
	Curve as SequoiaCurve, Features, HashAlgorithm, KeyFlags, SignatureType,
	SymmetricAlgorithm,
};
use sequoia_openpgp::{Cert, Packet};

use super::{
	sha1_to_hex, Algorithms, ArmoredKey, Backend, CipherSuite, Curve, PGPError,
	Rsa, UniversalError, UserID,
};
...
fn generate_key(
	...
	let wrapped_key: Result<Key4<SecretParts, PrimaryRole>, UniversalError> =
		match algorithm {
			...
			Algorithms::Ecc(curve) => match curve {
				Curve::Ed25519 => {
					Key4::generate_ecc(for_signing, SequoiaCurve::Ed25519)
				}
				Curve::Cv25519 => {
					Key4::generate_ecc(for_signing, SequoiaCurve::Cv25519)
				}
				Curve::NistP256 => {
					Key4::generate_ecc(for_signing, SequoiaCurve::NistP256)
				}
				Curve::NistP384 => {
					Key4::generate_ecc(for_signing, SequoiaCurve::NistP384)
				}
				Curve::NistP521 => {
					Key4::generate_ecc(for_signing, SequoiaCurve::NistP521)
				}
			},
		};
	...
	fn get_armored_results(
		...
		let direct_key_signature =
			SignatureBuilder::new(SignatureType::DirectKey)
				...
				.sign_direct_key(
					&mut signer,
					Some(primary_key_packet.parts_as_public()),
				)?;
		...
		if let Some(uid_string) = uid.get_id() {
			let uid_signature_builder =
				SignatureBuilder::from(direct_key_signature)
					.set_signature_creation_time(creation_time)?
			...
			let uid_signature =
				uid_packet.bind(&mut signer, &cert, uid_signature_builder)?;
		...
		// Encryption subkey
		let mut subkey = generate_key(
			self.cipher_suite.get_encryption_key_algorithm(),
			false,
		)?
		.parts_into_secret()?
		...
		let subkey_signature_builder =
			SignatureBuilder::new(SignatureType::SubkeyBinding)
				...
				.set_key_validity_period(None)?;
		let subkey_signature =
			subkey_packet.bind(&mut signer, &cert, subkey_signature_builder)?;
		cert = cert.insert_packets(vec![
		...
		if cert.unknowns().next().is_none() {
			...
			let armored_public_key =
				String::from_utf8(SerializeInto::to_vec(&cert.armored())?)?;
			let private_hex = SerializeInto::to_vec(&cert.as_tsk())?;
			let mut private_key_writer =
				Writer::new(Vec::new(), Kind::SecretKey)?;
			private_key_writer.write_all(&private_hex)?;
			let armored_private_key =
				String::from_utf8_lossy(&private_key_writer.finalize()?)
					.to_string();
...
impl SequoiaBackend {
	/// Create new instance
	pub fn new<C: Into<CipherSuite>>(
		cipher_suite: C,
	) -> Result<Self, PGPError> {
		let ciphers = cipher_suite.into();
		let primary_key =
			generate_key(ciphers.get_signing_key_algorithm(), true)?;
		...
		let mut public_key_buffer = MarshalInto::to_vec(primary_key.mpis())
...
mod sequoia_backend_test {
	...
	fn ed25519_fingerprint_calculation() {
		...
		let fingerprint_sequoia =
			backend.get_primary_key().fingerprint().to_hex();
	...
	fn ed25519_shuffle() {
		...
		primary_key
			.set_creation_time(
				UNIX_EPOCH.clone() + Duration::from_secs(timestamp_before),
			)
			.unwrap();
		let fingerprint_sequoia_before = primary_key.fingerprint().to_hex();
		primary_key
			.set_creation_time(
				UNIX_EPOCH.clone() + Duration::from_secs(timestamp_after),
			)
			.unwrap();
